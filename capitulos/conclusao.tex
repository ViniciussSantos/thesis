...
\section*{Sobre o Uso de Inteligência Artificial Generativa}

Esta seção descreve, de forma transparente e delimitada, o uso de ferramentas de inteligência artificial generativa durante a elaboração deste trabalho, com o objetivo de assegurar a integridade metodológica e a adequada atribuição de responsabilidades no desenvolvimento do Escalonador Adaptativo proposto.

O uso dessas ferramentas foi restrito a atividades de caráter auxiliar e não decisório, tendo sido empregado exclusivamente nas seguintes etapas:

\begin{itemize}
  \item \textbf{Levantamento exploratório preliminar}: ferramentas de IA generativa foram utilizadas de forma limitada para sugerir possíveis referências bibliográficas e temas correlatos. Todas as fontes efetivamente utilizadas neste trabalho foram posteriormente verificadas de maneira independente, por meio de acesso direto aos materiais originais.
  
  \item \textbf{Apoio à compreensão inicial}: recursos de IA generativa foram empregados apenas como suporte complementar para a compreensão preliminar de aspectos do funcionamento interno do RocksDB. Tal uso não substituiu a análise direta de documentação oficial, código-fonte e literatura técnica pertinente.
  
  \item \textbf{Organização estrutural preliminar}: ferramentas de IA generativa foram utilizadas para obter sugestões iniciais de organização textual. A estrutura final da dissertação foi definida, revisada e validada integralmente pelo autor.
\end{itemize}

Destaca-se que as ferramentas de inteligência artificial generativa não foram utilizadas na redação final do texto, na formulação de contribuições técnicas, na implementação das soluções propostas, nem na análise dos resultados. Todas as decisões metodológicas, interpretações, implementações e conclusões apresentadas são de inteira responsabilidade do autor.
