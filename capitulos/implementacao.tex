...
\node[box, fill=green!15, below=of picker] (utility)
    {\textbf{Calculador de Utilidade} ($A^{\text{boost}}_i$)};
\node[box, fill=green!15, below=of utility] (observer)
...
% Arrows
\draw[arrow] (rocksdb) -- node[above, font=\tiny]{flush/pick}
                          node[below, font=\tiny]{decisão} (picker);
\draw[arrow] (observer) -- (utility);
\draw[arrow] (utility)  -- (picker);
\draw[arrow] (memtable) -- (sstable);
\draw[arrow] (sstable)  -- (pmem);
\draw[dasharrow] (pmem.east)  -- node[above, font=\tiny]{latência} (observer.west);
\draw[dasharrow] (cpu.north)  -- (observer.south);
